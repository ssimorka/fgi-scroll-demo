...
Company Milestones (`milestones.html`, after 35pharma's Pipeline page): sequencing-clip hero → "01" overview panel → "02" roadmap: a `.chart` card (the Chartist horizontal bar chart from the community kit, redrawn in FGI type and the brand data-viz palette: title + secondary line over a rule, category labels left, thin bars over a dashed grid, ticks below, dot legend), rows grouped by phase (Research, Validation, Launch preparation, Launch), each bar reaching the stage the milestone is at (planned / under way / complete) with its status as the value label. Bars draw in from the left on scroll. `.chart` is reusable: `--v` (0-1) is a bar's length, `.s1/.s2/.s3` the series colours. The seven milestones and the funding line are from the investor deck; the phase grouping is ours.
