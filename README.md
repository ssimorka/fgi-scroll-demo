...
Company Milestones (`milestones.html`, after 35pharma's Pipeline page): sequencing-clip hero → "01" overview panel → "02" roadmap chart: one card per phase (Research, Validation, Launch preparation, Launch), a row per milestone with a bar drawn across the stages reached (planned / under way / complete), stage labels on the header row, a legend instead below 1024. Bars draw in from the left on scroll. The seven milestones and the funding line are from the investor deck; the phase grouping is ours.
