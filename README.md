...
Mission, Vision & Values (`company.html`): photo hero → "01" mission statement panel → "02" vision (split heading / copy, photo card) and "03" values (the four core principles as tiles, each with a Design Kit icon: Layers, Cpu, Activity, Shield, kept in `assets/icons/`) with a founders band linking to Leadership. Copy is FGI's own, from the old site and the investor deck (mission pillars, founder titles).
