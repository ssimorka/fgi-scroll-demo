(() => {
  ...
  if (!reduceMotion) {
    ...
    ScrollTrigger.batch('[data-reveal]', {
      ...
      onEnter: (batch) => gsap.to(batch, { opacity: 1, y: 0, duration: 1, ease: 'power3.out', stagger: .08, overwrite: true,
        onComplete: () => batch.forEach(el => { el.classList.add('is-in'); gsap.set(el, { clearProps: 'transform' }); }) }), // .is-in releases the CSS start state, then the inline transform goes so CSS hover transforms apply
    });
  }
  ...
  // scrubbed to exactly that overlap. Tablet and up (a panel taller than the
  // viewport pins on its bottom edge); phones scroll plainly, since a covering panel
  // had to be held to a full viewport, which left blank ground under short sections.
  // data-slide on the .stack-over (0-1) slows the cover to that
